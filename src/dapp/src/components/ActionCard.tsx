...
export const choseField = (type: String) => {
  if (type == "bigint | number" || type == "number | bigint")
    return AmountField;
  switch (type) {
    case "Address":
      return AddressField;
...
export const ActionCard: React.FC<ActionCardProps> = ({
  ...
  const _defaultParams = isDeploy
    ? { ...methodParams, ...deploy?.configType }
    : methodParams;
  const [enteredParams, setEnteredParams] = useState<ParamsWithValue>(
    _defaultParams as ParamsWithValue,
  );
  ...
  const [outNames, setOutNames] = useState<string[]>(
    "outNames" in methodConfig ? methodConfig.outNames : [],
  );
  ...
  useEffect(() => {
    function processParams(params: Parameters): JSX.Element[] {
      ...
      for (let [paramName, { type, defaultValue, optional }] of Object.entries(
        params,
      )) {
        ...
        if (Field == UnknownField) {
          ...
          if (types.length > 1) {
            fields.push(
              <MultiTypeField key={paramName} {...props} types={types} />,
            );
            continue;
          }
        }
  ...
  const inactiveButtonText = () => {
    if (correctParams.length !== Object.keys(enteredParams).length)
                ...
                {stringifyResult(getResult).map(({ name, strValue }) => (
                  <Box key={name} mt="2">
                    ...
                    >
                      {name ? (
                        <>
                          <Badge key={name + "_badge"}>{name}: </Badge>{" "}
                          {strValue}
                        </>
                      ) : (
                        <Center>
